...
export function DatabaseConnectionsDialog({ trigger, onConnectionAdded }: DatabaseConnectionsDialogProps) {
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8081/api/connections/test', {
        method: 'POST',
        headers: {
  ...
  const saveConnection = async () => {
    ...
    try {
      const url = editingConnection ? 
        'http://localhost:8081/api/connections/update' : 
  ...
  const removeConnection = async (id: string) => {
    try {
      const response = await fetch('http://localhost:8081/api/connections/remove', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
